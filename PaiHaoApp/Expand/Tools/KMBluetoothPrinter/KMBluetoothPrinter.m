...
@implementation KMBluetoothPrinter
 ...
 */
- (void)setAlignment:(KM_TextAlignment)alignment{
 ...
 *  @param fontSize 字号
 */
- (void)setFontSize:(KM_FontSize)fontSize{
...
#pragma mark  文字
- (void)appendText:(NSString *)text alignment:(KM_TextAlignment)alignment{
    [self appendText:text alignment:alignment fontSize:KM_FontSizeTitleSmalle];
}

- (void)appendText:(NSString *)text alignment:(KM_TextAlignment)alignment fontSize:(KM_FontSize)fontSize{
    // 1.文字对齐方式
    ...
    // 4.换行
    [self appendNewLine];
    if (fontSize != KM_FontSizeTitleSmalle) {
        [self appendNewLine];
    }
}

- (void)appendTitle:(NSString *)title value:(NSString *)value{
    [self appendTitle:title value:value fontSize:KM_FontSizeTitleSmalle];
}

- (void)appendTitle:(NSString *)title value:(NSString *)value fontSize:(KM_FontSize)fontSize{
    // 1.设置对齐方式
    [self setAlignment:KM_TextAlignmentLeft];
    // 2.设置字号
    [self setFontSize:fontSize];
    // 3.设置标题内容
    [self setText:title];
...
- (void)appendTitle:(NSString *)title value:(NSString *)value valueOffset:(NSInteger)offset{
    [self appendTitle:title value:value valueOffset:offset fontSize:KM_FontSizeTitleSmalle];
}

- (void)appendTitle:(NSString *)title value:(NSString *)value valueOffset:(NSInteger)offset fontSize:(KM_FontSize)fontSize{
    // 1.设置对齐方式
    [self setAlignment:KM_TextAlignmentLeft];
    // 2.设置字号
    [self setFontSize:fontSize];
...
- (void)appendLeftText:(NSString *)left middleText:(NSString *)middle rightText:(NSString *)right isTitle:(BOOL)isTitle{
    [self setAlignment:KM_TextAlignmentLeft];
    [self setFontSize:KM_FontSizeTitleSmalle];
 NSInteger offset              = 0;
    if (!isTitle) {
 offset                        = 10;
    }
...
#pragma mark 图片
- (void)appendImage:(UIImage *)image alignment:(KM_TextAlignment)alignment maxWidth:(CGFloat)maxWidth{
    if (!image) {
        return;
    }
    // 1.设置图片对齐方式
    ...
    // 2.设置图片
 UIImage *newImage             = [image imageByResizeToSize:CGSizeMake(maxWidth, maxWidth) contentMode:UIViewContentModeScaleToFill];
...
- (void)appendBarCodeWithInfo:(NSString *)info{
    [self appendBarCodeWithInfo:info alignment:KM_TextAlignmentCenter maxWidth:500];
}

- (void)appendBarCodeWithInfo:(NSString *)info alignment:(KM_TextAlignment)alignment maxWidth:(CGFloat)maxWidth{
...
- (void)appendQRCodeWithInfo:(NSString *)info size:(NSInteger)size{
    [self appendQRCodeWithInfo:info size:size alignment:KM_TextAlignmentCenter];
}

- (void)appendQRCodeWithInfo:(NSString *)info size:(NSInteger)size alignment:(KM_TextAlignment)alignment{
    [self setAlignment:alignment];
...
- (void)appendQRCodeWithInfo:(NSString *)info{
    [self appendQRCodeWithInfo:info centerImage:nil alignment:KM_TextAlignmentCenter maxWidth:250];
}

- (void)appendQRCodeWithInfo:(NSString *)info centerImage:(UIImage *)centerImage alignment:(KM_TextAlignment)alignment maxWidth:(CGFloat )maxWidth{
...
- (void)appendSeperatorLine{
    // 1.设置分割线居中
    [self setAlignment:KM_TextAlignmentCenter];
    // 2.设置字号
    [self setFontSize:KM_FontSizeTitleSmalle];
    // 3.添加分割线
 NSString *line                = @"- - - - - - - - - - - - - - - -";
...
- (void)appendFooter:(NSString *)footerInfo{
 ...
 footerInfo                    = @"谢谢惠顾，欢迎下次光临！";
    ...
    [self appendText:footerInfo alignment:KM_TextAlignmentCenter];
}

- (NSData *)finalPrinterData{
    return _printerData;
}
